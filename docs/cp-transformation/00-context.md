...
| D44 | **Industries set = the four `hasPage` industries only, for now:** **B2B & Professional Services** (`b2b-services`), **Technology & SaaS** (`saas-companies`), **Ecommerce Brands** (`ecommerce-brands`), **Charities & Non-profits** (`charities-and-foundation`). The industries type's **duplicate slugs and mismatched `hasPage` flags are cleaned at CP-08.** The **five orphaned legacy industry-solutions** (no page in this set) redirect **individually to the closest relevant page, not the /industries hub** (a hub redirect is a soft dead end): interiors-and-furnishings, restaurants, driving-schools, pharmacies → `/services/web-design-development` (each is a website/lead-gen build, not an ecommerce brand — content-checked 19 Aug); sme-founders → `/solutions` hub (audience positioning, not an industry). | Ships a real, small industries set now; the taxonomy mess (O13 note) is a CP-08 cleanup, not a CP-02 blocker. Supersedes the old O6 shortlist framing. Redirect-to-closest-page follows the brief's rule against mass hub redirects. Inherited by CP-08/CP-15. |
