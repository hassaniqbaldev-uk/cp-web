...
| D44 | **Industries set = the four `hasPage` industries only, for now:** **B2B & Professional Services** (`b2b-services`), **Technology & SaaS** (`saas-companies`), **Ecommerce Brands** (`ecommerce-brands`), **Charities & Non-profits** (`charities-and-foundation`). The industries type's **duplicate slugs and mismatched `hasPage` flags are cleaned at CP-08.** | Ships a real, small industries set now; the taxonomy mess (O13 note) is a CP-08 cleanup, not a CP-02 blocker. Supersedes the old O6 shortlist framing. Inherited by CP-08. |
