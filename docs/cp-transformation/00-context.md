...
| **O20** | **Book a Call still appears outside the header/footer (found 21 Aug 2026 while removing it from the global chrome).** Header, HomeHeader, MobileMenu and Footer are now clean (→ "Start a project" / removed). But the Cal.com Book-a-Call closing CTA still renders via **`Cta.jsx`** on **home, about, case-studies (index + detail), how-we-work, partner-with-us, solutions**; via **`TheSolution`** on case-study detail pages; via **`Cta2`** ("Get Free Consultation" + Cal embed) on the **14 legacy service pages**; and on the **retiring wordpress LP** (`LpFooter`/`LpHeader`). `StickyCta.jsx` has it but is unused. So **"no Book a Call anywhere" is not yet fully true.** Replacing these is a separate site-wide closing-CTA change (what enquiry CTA replaces the Cal booking section) — a design decision, not done unilaterally. | Hassan (decision) | A genuinely Book-a-Call-free site |
