...
### Added after the CP-02 IA review, 19 August 2026

| ID | Decision | Rationale |
| --- | --- | --- |
| D40 | **The main navigation is exactly seven items, in this order: Work, Services, Solutions, Industries, Blog, About, Contact.** **Free Audit** is a **secondary link, not a nav item**, and **stays off service pages** (per the /audit decision). **Book a Call is removed entirely** (D8). | Confirmed by Hassan — this is the definitive list; **stop reconstructing it.** Inherited by CP-03 (nav build). The mega-menu columns under Services are the four pillars (data-driven). |
| D41 | **Paid Media slug** — `/services/ppc` **→ `/services/paid-media`**; `/services/ppc` **REDIRECTS** at CP-15. User-facing name is **Paid Media** (navLabel updated on staging); **"PPC" terminology stays in the body copy** for search. | The slug should match the user-facing name; PPC keeps its search value in copy, not the URL. Inherited by CP-09 (copy) and CP-15 (redirect). |
| D42 | **Ongoing Growth & Support slug** — the maintenance parent (D36) uses **`/services/growth-and-support`**; `/services/maintenance` **REDIRECTS** to it at CP-15. | Shorter and clearer than the full phrase. Inherited by CP-05/CP-15. |
| D43 | **Migrations, Accessibility and Speed sit under Web & Ecommerce**, not Growth & Performance. (Speed added during Batch B, 21 Aug 2026 — site speed is a build concern and belongs alongside accessibility and migrations.) | They are **build and delivery capabilities**, not growth services. Resolves the cross-cutting flag from CP-02 §8. Inherited by CP-03 (pillar map). |
| D44 | **Industries — evidence-led (Claude worked out the set from the 31 case-study bodies; Hassan approved 19 Aug).** **SEVEN ship** (all with real delivered evidence): B2B & Professional Services, Technology & SaaS, Ecommerce Brands, Charities & Non-profits, **Education & EdTech** (added), **Travel, Hospitality & Leisure** (added), Home Improvement & Interiors. **THREE HELD** as `hasPage:false` docs — **Driving Schools, Pharmacies, Restaurants** — NOT published: they have **no delivered case study**, and the "Recent work" fallback would fill their page with unrelated work (a pharmacy owner seeing a safari lodge + a jeweller learns we've never done pharmacies — worse than no page). Hassan is deciding whether we actively chase those verticals; until then their legacy `/solutions/<x>` URLs redirect to `/services/web-design-development`, and turning one on later is a **content change** (`hasPage:true` → repoints to its own page). `interiors-and-furnishings` → `/industries/home-improvement-interiors`; `sme-founders` → `/solutions` (audience positioning). Full mapping + expansion contract in `04-information-architecture.md` §3.4/§4. | Evidence-led, honest set: never fake a vertical we've not worked in, but ship the ones we have. `hasPage`-driven so expansion is content-only. The fallback is a safety net for served verticals, not a way to invent unserved ones. Duplicate-slug/flag cleanup is CP-08. Inherited by CP-08/CP-15. |

**navLabel corrections applied to staging (19 Aug 2026):** `wordpress` "Wordpress" → **"WordPress"**;
`ppc` "PPC" → **"Paid Media"**.
...
| ~~O9~~ | **MOOT (25 Aug 2026).** It existed to validate whether removing Book a Call was right; Book a Call is already removed from every page, so this no longer blocks anything. Closeable. | Hassan | — |
| ~~O12~~ | **RESOLVED (25 Aug 2026).** All 32 published case-study slugs are clean/normalised (lowercase, hyphenated, no spaces); case-study URLs use the case-study slug and taxonomy slugs never appear in a URL. No malformed slug in any live URL. | Claude Code | — |
| O14 | **CLOSED (19 Aug 2026) — YES, we deliver standalone security.** Hassan confirmed CP delivers it as a service, including **malware removal, firewall configuration and monitoring.** `/services/security` **stays a service page under Growth & Performance**; the meta description is **restored** (rules-compliant version written to staging, reflecting exactly those capabilities). Overrides the D36 "demote to capability" default. | Hassan | — |
| ~~O15~~ | **DECIDED (25 Aug 2026, Hassan): KEEP.** A legitimate goal even if none delivered under that name; Custom App Development is the service behind it. Stays in the goal-solutions set of 4. | Hassan | — |
| ~~O16~~ | **DONE (25 Aug 2026, Hassan): the 6 placeholder stubs (`loop`, `lola-blake`, `core-estates`, `amana-partnership`, `drive-uk`, `ofh-care`) DELETED from staging (0 references, draft-only); `ivy-and-duke` PUBLISHED (complete: thumbnail, excerpt, hero — live at /case-studies/ivy-and-duke). Published case-study count 32 → 33.** `biome4pets` already published. | Claude Code (done) | — |
| ~~O17~~ | **DECIDED (25 Aug 2026, Hassan): leave as-is — no outbound links, not featured.** Already the decision; no change needed. | Hassan | — |
| ~~O18~~ | **RESOLVED (25 Aug 2026): no service has `options.pricingCard` any more (unapproved pricing gone) and `projectShowcase` is now proper per-service fit/not-fit cards (repeated placeholder trio gone). NB `modularLayout` is now ON for all 18 services, so this renders — but the data was cleaned, so it renders correctly. Original note:** **Service-doc content cleanup pass (found during CP-05, 21 Aug 2026).** Across the 15 real `services` docs, two modules hold **hidden, unrendered, low-quality data** (they only render when `modularLayout` is on, which is the two new pages only): (a) **`projectShowcase` is a repeated placeholder** — the same Smokey Carter / Game Art Brain / Ivy & Duke trio on nearly every doc, not curated per service; (b) **`options.pricingCard` holds unapproved, inconsistent pricing** (e.g. £1,995 / £2,495 / £1k-mo — none matching the approved £1,500 / £3,500). Neither renders today. Clean or remove before any of the 14 legacy pages is ever switched to `modularLayout`. | Hassan / content | Switching any legacy service page to the modular layout |
| ~~**O20**~~ | **CLOSED for PAGES (25 Aug 2026) — this note was STALE.** A full render sweep of every route (SSR + live DOM after hydration) shows **no Book a Call, no Cal.com embed and no booking CTA on any page except `/call` itself.** `Cta.jsx`, `Cta2` and `TheSolution` all already use **"Start a project" → /contact**; the retiring wordpress LP was deleted this session; `StickyCta` is unused. **0 page changes were needed.** REMAINING (one surface only): the **email templates** — `customer-template.js` (live on the contact + audit thank-yous) and `lp-customer-template.js` (orphaned, its LP frontend is deleted) contain a booking-built section ("book a quick discovery call" + a "Schedule a Quick Call" button → /call). That section is built around booking, so per Hassan's rule it is FLAGGED not rewritten. Proposed fix: button → "Start a project" / /contact, and soften the paragraph off "discovery call". If Hassan is seeing Book a Call live, that is PRODUCTION (old code, not yet deployed); development is clean and it goes live at cutover. | Hassan (confirm email swap) | A genuinely Book-a-Call-free site |
| ~~**O19**~~ | **RESOLVED (25 Aug 2026): the `wordpress` doc now holds correct WordPress copy in fit/notFit/options; the UI-UX contamination is gone. It renders (modularLayout on) and renders correctly. Original note:** **CONTENT ERROR (not just untidiness) — flagged separately per Hassan.** The **`wordpress`** service doc is **cross-contaminated with UI/UX-design copy**: its `projectShowcase.fitCard` / `notFitCard` / `options` read as UI/UX content ("Startups needing an MVP design to raise capital", "SaaS companies with outdated interfaces", "just a logo — see Branding", "print-only design requests"). This is wrong content in the wrong document. It does not render today (WordPress is not `modularLayout`), but it must be corrected, and the other docs spot-checked for the same paste errors. | Hassan / content | Correctness of the `wordpress` doc before it is ever surfaced |
