...
| O16 | **The 8 draft-only case studies — INSPECTED 19 Aug (see status log).** Reality: **only `ivy-and-duke` has real content** (completed its SEO; kept as draft). **6 are placeholder stubs cloned from Casa Botanica** (`loop`, `lola-blake`, `core-estates`, `amana-partnership`, `drive-uk`, `ofh-care`) — real title/slug/live-URL/service tag but Casa Botanica boilerplate bodies; left untouched, not padded. `biome4pets` is the 8th (ours). **Need from Hassan:** a real brief per stub (as for Biome4Pets) to build them, or a decision to discard. | Hassan | CP-11/CP-12 evidence set |
